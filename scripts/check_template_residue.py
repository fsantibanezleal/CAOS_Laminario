...
The archetype template ships an intentional example PIPELINE (not a package, not a lab)
(`data-pipeline/pipeline/`, an SIR model, `EX0*` baked cases) so a fresh clone runs end to end.
Instantiation is supposed to REPLACE all of it with the real product; five shipped products proved
that replacement is easy to forget, leaving
...
FORBIDDEN_PATH_SUBSTR = (
    ...
    # Laminario deploys as a server application; a Pages workflow would be a second, undeclared
    # publish path for the same repository.
